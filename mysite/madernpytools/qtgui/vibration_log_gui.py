...
from madernpytools.qtgui.ui_vibration_log_gui import Ui_VibrationLogGui
...
class VibrationLogGui(AbstractMainWindow, Ui_VibrationLogGui, mbb.IXML, mbb.HasTraitLinks):
    ...
    def __init__(self,
        ...
        # Data logger:
        traitlets.link((self.data_logger, 'log'), (self.log_control, 'log'))
    ...
    def _update_available_keys(self):
        ...
        self.time_display.signal_selection.selected_items = new_list
    ...
    def from_xml(root, *args, **kwargs):
        ...
        return VibrationLogGui(**item_dict)
...
def get_default_logger():
    ...
    sensors = mdaq.SensorList(ai_list=[*acc_sensors],
                              )
    configuration = mdaq.TaskConfiguration(sampling_rate=10240, buffer_size=102400,
                                           chassis_name=chassis_name, task_name='AccTask',
                                           )
...
def get_default_gui():
    ...
    # Get task
    ni_logger = get_default_logger()

    # Define data GUI
    data_gui = VibrationLogGui(data_logger=ni_logger, buffer_size=ni_logger.task.sampling_rate*10)
...
def main(settings_file: str=''):
    ...
    loop = QEventLoop(app)
    filename = settings_file if settings_file !='' else f'{mod_dir}/../settings/vibration_settings.xml'
    if not os.path.exists(filename):
        data_gui = get_default_gui()
    ...
    # Overwrite log:
    data_gui.data_logger.log = mlog.Log(log_info=mlog.LogInfo(description='Vibration measurement',
                                                              sampling_rate=data_gui.data_logger.task.sampling_rate,
                                                              signal_header=data_gui.data_logger.task.added_keys)
                                        )

    data_gui.showMaximized()
...
if __name__ == "__main__":
    ...
    mlogging.logger.addHandler(stream)
